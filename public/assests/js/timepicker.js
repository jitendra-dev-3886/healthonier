...
document.addEventListener("DOMContentLoaded", () => {
  ...
  tp.attach({
    target: document.getElementById("demoA"),
 after: time => alert("Starting time: " + time) // run function after selecting time
  });

  tp.attach({
    target: document.getElementById("demoB"),
    after: time => alert("Cloasing time: " + time) // run function after selecting time
  });

  tp.attach({
    target: document.getElementById("demoC"),
    after: time => alert("Input C time: " + time) // run function after selecting time
  });
});